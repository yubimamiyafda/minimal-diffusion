...
def main():
    ...
    torch.manual_seed(args.seed + os.environ['LOCAL_RANK'])
    # np.random.seed(args.seed + args.local_rank)
    np.random.seed(args.seed + os.environ['LOCAL_RANK'])
    # if args.local_rank == 0:
    if os.environ['LOCAL_RANK'] == 0:
        print(args)
    ...
    if ngpus > 1:
        ...
        # should i replace above line with below lines?
        # rank = args.nr * args.gpus + gpu
        # i think -nr may have an equivilant of -lr or -local-rank in passed arguments already
        # torch.distributed.init_process_group(backend='nccl', init_method='env://', world_size=args.world_size, rank=rank)
        # model = DDP(model, device_ids=[args.local_rank], output_device=args.local_rank)
        model = DDP(model, device_ids=[os.environ['LOCAL_RANK']], output_device=os.environ['LOCAL_RANK'])
